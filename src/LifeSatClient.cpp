...
std::string LifeSatClient::GetBuildInRecorderObjectID()
{
  std::string result = "";
  ...
  if ((status = m_lifesatRemoteCommunication->GetPlaybackObject(getPlaybackObjectRequest, getPlaybackObjectResponse,
      NULL)) == LIFESAT_REMOTE_STATUS_OK)
  {
        ...
        it < getPlaybackObjectResponse.GetPlaybackContainers().end(); it++)
    {
      ...
      if (strcmp(container->SourceID.c_str(), LIFESAT_BUILD_IN_RECORDER_SOURCE_ID) == 0)
      {
...
LifeSatClient::LifeSatClient(CHelper_libXBMC_addon* xbmc, CHelper_libXBMC_pvr* pvr, CHelper_libKODI_guilib* gui,
    ...
    std::string password, bool add_episode_to_rec_title, bool group_recordings_by_series, bool no_group_single_rec)
{
  ...
  LifeSatRemoteStatusCode status;
  if ((status = m_lifesatRemoteCommunication->GetToken(NULL)) == LIFESAT_REMOTE_STATUS_OK)
  {
  ...
  ServerInfo si;
  if ((status = m_lifesatRemoteCommunication->GetServerInfo(server_info_request, si, NULL)) == LIFESAT_REMOTE_STATUS_OK)
  {
  ...
  status = m_lifesatRemoteCommunication->GetStreamingCapabilities(streamin_caps_request, streaming_caps, NULL);
  if (status == LIFESAT_REMOTE_STATUS_OK)
    transcoding_supported_ = streaming_caps.IsTranscoderSupported(
        lifesatremote::StreamingCapabilities::STREAMING_TRANSCODER_H264);

  GetFavoritesRequest favorites_request;
  status = m_lifesatRemoteCommunication->GetFavorites(favorites_request, channel_favorites_, NULL);
  favorites_supported_ = (status == LIFESAT_REMOTE_STATUS_OK);
  ...
  std::string error;
  if ((status = m_lifesatRemoteCommunication->GetChannels(request, *m_channels, &error)) == LIFESAT_REMOTE_STATUS_OK)
  {
    ...
    XBMC->Log(LOG_INFO, "Connected to LifeSat Server '%s'", m_hostname.c_str());
    if (m_showinfomsg)
    {
    ...
    m_recordingsid_by_date.append(LIFESAT_RECODINGS_BY_DATA_ID);

    m_recordingsid_by_series = m_recordingsid;
    m_recordingsid_by_series.append(LIFESAT_RECODINGS_BY_SERIES_ID);
  ...
  else
  {
    ...
    XBMC->Log(LOG_ERROR,
        "Could not connect to LifeSat Server '%s' on port '%i' with username '%s' (Error code : %d Description : %s)",
        hostname.c_str(), port, username.c_str(), (int) status, error.c_str());
  }
}

void *LifeSatClient::Process()
{
  XBMC->Log(LOG_DEBUG, "LifeSatUpdateProcess:: thread started");
  ...
  XBMC->Log(LOG_DEBUG, "LifeSatUpdateProcess:: thread stopped");
  return NULL;
}

bool LifeSatClient::GetStatus()
{
  return m_connected;
}

int LifeSatClient::GetChannelsAmount()
{
  return m_channelMap.size();
}

PVR_ERROR LifeSatClient::GetChannels(ADDON_HANDLE handle, bool bRadio)
{
...
int LifeSatClient::GetChannelGroupsAmount(void)
{
...
PVR_ERROR LifeSatClient::GetChannelGroups(ADDON_HANDLE handle, bool bRadio)
{
  if (!favorites_supported_)
...
PVR_ERROR LifeSatClient::GetChannelGroupMembers(ADDON_HANDLE handle, const PVR_CHANNEL_GROUP &group)
{
  if (!favorites_supported_)
    return PVR_ERROR_NOT_IMPLEMENTED;
  ...
  for (size_t i = 0; i < channel_favorites_.favorites_.size(); i++)
  {
    ...
    lifesatremote::ChannelFavorite::favorite_channel_list_t chlist = channel_favorites_.favorites_[i].get_channels();

    for (size_t j = 0; j < chlist.size(); j++)
    {
      if (inverse_channel_map_.find(chlist[j]) != inverse_channel_map_.end())
      {
        lifesatremote::Channel* ch = m_channelMap[inverse_channel_map_[chlist[j]]];

        bool isRadio = (ch->GetChannelType() == lifesatremote::Channel::CHANNEL_TYPE_RADIO);
...
PVR_ERROR LifeSatClient::GetTimerTypes(PVR_TIMER_TYPE types[], int *size)
{
...
int LifeSatClient::GetTimersAmount()
{
  return m_timerCount;
}

int LifeSatClient::GetInternalUniqueIdFromChannelId(const std::string& channelId)
{
  for (std::map<int, Channel*>::iterator it = m_channelMap.begin(); it != m_channelMap.end(); ++it)
  {
    Channel * channel = (*it).second;
...
std::string LifeSatClient::make_timer_hash(const std::string& timer_id, const std::string& schedule_id)
{
  std::string res = schedule_id + "#" + timer_id;
  return res;
}

bool LifeSatClient::parse_timer_hash(const char* timer_hash, std::string& timer_id, std::string& schedule_id)
{
  bool ret_val = false;
...
unsigned int LifeSatClient::get_kodi_timer_idx_from_lifesat(const std::string& id)
{
...
int LifeSatClient::GetSchedules(ADDON_HANDLE handle, const RecordingList& recordings)
{
  ...
  LifeSatRemoteStatusCode status;
  std::string error;
  if ((status = m_lifesatRemoteCommunication->GetSchedules(request, response, &error)) != LIFESAT_REMOTE_STATUS_OK)
  {
  ...
  for (size_t i = 0; i < manual_schedules.size(); i++)
  {
    ...
    {
      unsigned int kodi_idx = get_kodi_timer_idx_from_lifesat(manual_schedules[i]->GetID());
  ...
  for (size_t i = 0; i < epg_schedules.size(); i++)
  {
    ...
    {
      unsigned int kodi_idx = get_kodi_timer_idx_from_lifesat(epg_schedules[i]->GetID());
  ...
  {
    unsigned int kodi_idx = get_kodi_timer_idx_from_lifesat(bp_schedules[i]->GetID());
...
PVR_ERROR LifeSatClient::GetTimers(ADDON_HANDLE handle)
{
  ...
  LifeSatRemoteStatusCode status;
  std::string error;
  if ((status = m_lifesatRemoteCommunication->GetRecordings(recordingsRequest, recordings, &error))
  ...
  for (size_t i = 0; i < recordings.size(); i++, index++)
  {
    ...
    if (schedule_map_.find(rec->GetScheduleID()) != schedule_map_.end())
    {
      ...
      switch (schedule_type)
      {
        ...
        case TIMER_CREATED_REPEATING_MANUAL:
          ...
          xbmcTimer.iParentClientIndex = get_kodi_timer_idx_from_lifesat(rec->GetScheduleID());
        break;
    ...
    xbmcTimer.iClientIndex = get_kodi_timer_idx_from_lifesat(rec->GetID());
    //misuse strDirectory to keep id of the timer
...
bool LifeSatClient::get_lifesat_program_id(std::string& channelId, int start_time, std::string& lifesat_program_id)
{
  bool ret_val = false;
  ...
  if (DoEPGSearch(epgSearchResult, channelId, start_time, start_time))
  {
    ...
    {
      lifesat_program_id = epgSearchResult.at(0)->GetEpgData().at(0)->GetID();
      ret_val = true;
    }
  }
...
PVR_ERROR LifeSatClient::AddTimer(const PVR_TIMER &timer)
{
  PVR_ERROR result = PVR_ERROR_FAILED;
  P8PLATFORM::CLockObject critsec(m_mutex);
  LifeSatRemoteStatusCode status;
  ...
  switch (timer.iTimerType)
  {
    ...
    case TIMER_REPEATING_MANUAL:
    {
      ...
      {
        //change day mask to LifeSat server format (Sun - first day)
    ...
    case TIMER_ONCE_EPG:
    {
      ...
      std::string lifesat_program_id;
      if (get_lifesat_program_id(channelId, timer.iEpgUid, lifesat_program_id))
      {
        addScheduleRequest = new AddScheduleByEpgRequest(channelId, lifesat_program_id, false, true, true,
    ...
    case TIMER_REPEATING_EPG:
    {
      ...
      if (get_lifesat_program_id(channelId, timer.iEpgUid, lifesat_program_id))
      {
        addScheduleRequest = new AddScheduleByEpgRequest(channelId, lifesat_program_id, record_series, newOnly, anytime,
            numberToKeep, marginBefore, marginAfter);
      }
    }
    break;
  ...
  {
    std::string error;
    if ((status = m_lifesatRemoteCommunication->AddSchedule(*addScheduleRequest, &error)) == LIFESAT_REMOTE_STATUS_OK)
    {
...
PVR_ERROR LifeSatClient::DeleteTimer(const PVR_TIMER &timer)
{
  PVR_ERROR result = PVR_ERROR_FAILED;
  P8PLATFORM::CLockObject critsec(m_mutex);
  LifeSatRemoteStatusCode status = LIFESAT_REMOTE_STATUS_ERROR;
  std::string error;
  ...
  switch (timer.iTimerType)
  {
    ...
    case TIMER_ONCE_KEYWORD_CHILD:
    {
      ...
      status = m_lifesatRemoteCommunication->RemoveRecording(removeRecording, &error);
    }
    break;
    case TIMER_CREATED_REPEATING_MANUAL:
    case TIMER_REPEATING_EPG:
    case TIMER_CREATED_REPEATING_KEYWORD:
    {
      ...
      status = m_lifesatRemoteCommunication->RemoveSchedule(removeSchedule, &error);
    }
    break;
  }
...
PVR_ERROR LifeSatClient::UpdateTimer(const PVR_TIMER &timer)
{
  ...
  if (schedule_id.size() > 0)
  {
    ...
    std::string error;
    LifeSatRemoteStatusCode status = m_lifesatRemoteCommunication->UpdateSchedule(update_request, &error);
...
int LifeSatClient::GetRecordingsAmount()
{

  return m_recordingCount;
}

std::string LifeSatClient::GetRecordedTVByDateObjectID(const std::string& buildInRecoderObjectID)
{
  std::string result = "";
  LifeSatRemoteStatusCode status;
...
PVR_ERROR LifeSatClient::DeleteRecording(const PVR_RECORDING& recording)
{
//  P8PLATFORM::CLockObject critsec(m_mutex);
  PVR_ERROR result = PVR_ERROR_FAILED;
  LifeSatRemoteStatusCode status;
  ...
  std::string error;
  if ((status = m_lifesatRemoteCommunication->RemovePlaybackObject(remoteObj, &error)) != LIFESAT_REMOTE_STATUS_OK)
  {
...
PVR_ERROR LifeSatClient::GetRecordings(ADDON_HANDLE handle)
{
  P8PLATFORM::CLockObject critsec(m_mutex);
  PVR_ERROR result = PVR_ERROR_FAILED;
  LifeSatRemoteStatusCode status;
  ...
  std::string error;
  if ((status = m_lifesatRemoteCommunication->GetPlaybackObject(getPlaybackObjectRequest, getPlaybackObjectResponse,
      &error)) != LIFESAT_REMOTE_STATUS_OK)
  {
...
bool LifeSatClient::GetRecordingURL(const char* recording_id, std::string& url, bool use_transcoder, int width, 
...
void LifeSatClient::GetDriveSpace(long long *iTotal, long long *iUsed)
{
//  P8PLATFORM::CLockObject critsec(m_mutex);
  ...
  *iUsed = 0;
  ...
  LifeSatRemoteStatusCode status;
  if ((status = m_lifesatRemoteCommunication->GetRecordingSettings(recordingsettingsrequest, settings, NULL))
      == LIFESAT_REMOTE_STATUS_OK)
  {
...
int LifeSatClient::GetCurrentChannelId()
{
  return m_currentChannelId;
}

bool LifeSatClient::StartStreaming(const PVR_CHANNEL &channel, StreamRequest* streamRequest, std::string& stream_url)
{
  LifeSatRemoteStatusCode status;
  std::string error;
  if ((status = m_lifesatRemoteCommunication->PlayChannel(*streamRequest, *m_stream, &error))
      != LIFESAT_REMOTE_STATUS_OK)
  {
...
bool LifeSatClient::OpenLiveStream(const PVR_CHANNEL &channel, bool use_timeshift, bool use_transcoder, int width,
    int height, int bitrate, std::string audiotrack)
{
  bool ret_val = false;
...
int LifeSatClient::ReadLiveStream(unsigned char *pBuffer, unsigned int iBufferSize)
{
  if (m_live_streamer)
...
long long LifeSatClient::SeekLiveStream(long long iPosition, int iWhence)
{
  if (m_live_streamer)
...
long long LifeSatClient::PositionLiveStream(void)
{
  if (m_live_streamer)
...
time_t LifeSatClient::GetPlayingTime()
{
  if (m_live_streamer)
...
time_t LifeSatClient::GetBufferTimeStart()
{
  if (m_live_streamer)
...
time_t LifeSatClient::GetBufferTimeEnd()
{
  if (m_live_streamer)
...
void LifeSatClient::StopStreaming(bool bUseChlHandle)
{
  P8PLATFORM::CLockObject critsec(m_mutex);
  ...
  LifeSatRemoteStatusCode status;
  std::string error;
  if ((status = m_lifesatRemoteCommunication->StopChannel(*request, &error)) != LIFESAT_REMOTE_STATUS_OK)
  {
...
void LifeSatClient::SetEPGGenre(lifesatremote::ItemMetadata& metadata, int& genre_type, int& genre_subtype)
{
...
bool LifeSatClient::DoEPGSearch(EpgSearchResult& epgSearchResult, const std::string& channelId, const long startTime,
    const long endTime, const std::string& programId)
{
  ...
  if ((status = m_lifesatRemoteCommunication->SearchEpg(epgSearchRequest, epgSearchResult, NULL))
      == LIFESAT_REMOTE_STATUS_OK)
  {
    return true;
  }
...
PVR_ERROR LifeSatClient::GetEPGForChannel(ADDON_HANDLE handle, const PVR_CHANNEL& channel, time_t iStart, time_t iEnd)
{
  PVR_ERROR result = PVR_ERROR_FAILED;
  P8PLATFORM::CLockObject critsec(m_mutex);
  Channel * c = m_channelMap[channel.iUniqueId];
...
LifeSatClient::~LifeSatClient(void)
{
  m_updating = false;
  if (IsRunning())
  {
  ...
  SAFE_DELETE(m_lifesatRemoteCommunication);
