...
#define LIFESAT_BUILD_IN_RECORDER_SOURCE_ID   "8F94B459-EFC0-4D91-9B29-EC3D72E92677"
#define LIFESAT_RECODINGS_BY_DATA_ID   "F6F08949-2A07-4074-9E9D-423D877270BB"
#define LIFESAT_RECODINGS_BY_SERIES_ID   "0E03FEB8-BD8F-46e7-B3EF-34F6890FB458"
...
enum lifesat_client_rec_showtype_e
{
...
class LifeSatClient: public P8PLATFORM::CThread, public lifesatremote::LifeSatRemoteLocker
{
public:
  LifeSatClient(ADDON::CHelper_libXBMC_addon* xbmc, CHelper_libXBMC_pvr* pvr, CHelper_libKODI_guilib* gui,
  ...
  bool GetStatus();
  bool StartStreaming(const PVR_CHANNEL &channel, lifesatremote::StreamRequest* streamRequest, std::string& stream_url);
...
private:
  bool DoEPGSearch(lifesatremote::EpgSearchResult& epgSearchResult, const std::string& channelId, const long startTime,
      const long endTime, const std::string & programId = "");
  void SetEPGGenre(lifesatremote::ItemMetadata& metadata, int& genre_type, int& genre_subtype);
  ...
  bool get_lifesat_program_id(std::string& channelId, int start_time, std::string& lifesat_program_id);
  int GetSchedules(ADDON_HANDLE handle, const lifesatremote::RecordingList& recordings);
  ...
  unsigned int get_kodi_timer_idx_from_lifesat(const std::string& id);
  ...
  lifesatremote::ILifeSatRemoteConnection* m_lifesatRemoteCommunication;
  bool m_connected;
  std::map<int, lifesatremote::Channel *> m_channelMap;
  lifesatremote::Stream * m_stream;
  int m_currentChannelId;
  lifesatremote::ChannelList* m_channels;
  long m_timerCount;
  ...
  lifesatremote::ChannelFavorites channel_favorites_;
